...
* **Multi-Model Support:** Currently available models are **[BirdNET v2.4](https://huggingface.co/justinchuby/BirdNET-onnx)** (ONNX and TFLite) and **[Google Perch v2.0](https://huggingface.co/justinchuby/Perch-onnx)** (ONNX). Custom models can be added via JSON configuration or through the UI.
...
1. **Clone and install dependencies:**
   ...
   npm install
   ```
   This automatically syncs the ONNX Runtime binaries and bundles the LiteRT Wasm worker via the `postinstall` script.
...
+-- src/tflite-inference-worker.js # Web Worker: LiteRT (TFLite) Wasm inference (source, bundled by esbuild)
+-- onnx/                          # ONNX Runtime Wasm binaries (synced from node_modules)
+-- tflite/                        # LiteRT Wasm worker bundle + Wasm binaries (generated by postinstall)
...
```
   Set `"format"` to `"onnx"` or `"tflite"` depending on the model file type.
...
Source code is distributed under the [GPL-3.0](LICENSE). Bundled runtimes and models have their own licenses:
